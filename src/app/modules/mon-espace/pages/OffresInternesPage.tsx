...
import { Building2, AlertCircle, Search, Filter, X, MapPin, Calendar, Clock, Briefcase, FileText, CheckCircle } from 'lucide-react';
...
export default function OffresInternesPage() {
  ...
  const loadContactHistory = async () => {
    ...
    try {
      const data = await getContactHistoryByType(JobOfferTypeEnum.INTERNAL);
  ...
  const filteredJobs = useMemo(() => {
    ...
    return result;
  }, [jobs, searchQuery, statusFilter, departmentFilter, deadlineFilter, sortBy]);
  ...
  const hasActiveFilters = searchQuery || statusFilter !== 'all' || departmentFilter !== 'all' || deadlineFilter !== 'all' || sortBy !== 'newest';
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        <PageBanner
          ...
          description={selectedJob.organizationName || ''}
        />
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <PageBanner
        icon={Building2}
        title={t('monEspace.banner.internalOffers.title')}
        description={t('monEspace.banner.internalOffers.description')}
        stats={[
          {
          ...
          <div className="bg-white rounded-lg border p-4 mb-6">
            ...
            {showFilters && (
              <div className="mt-4 pt-4 border-t grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {hasActiveFilters && (
                  <div className="md:col-span-2 lg:grid-cols-4 flex justify-end">
                    <Button
                      variant="ghost"
