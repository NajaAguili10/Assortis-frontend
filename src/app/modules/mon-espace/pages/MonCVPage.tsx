...
const MonCVPage = () => {
  ...
  useEffect(() => {
    const loadCVData = () => {
      // Mock: Check if user has a CV in the database
      // In production, this would be an API call
      const userCV = expertsDataService.getCurrentUserCV();
      ...
      if (userCV) {
        ...
        setFormData({
          ...
          bio: userCV.bio || '',
          title: userCV.title || '',
          level: userCV.level || 'JUNIOR',
          yearsExperience: userCV.yearsExperience || '0-2',
          sectors: userCV.sectors || [],
          subsectors: userCV.subsectors || [],
          skills: userCV.skills || [],
          availability: userCV.availability || 'NOT_AVAILABLE',
          availableFrom: userCV.availableFrom || '',
          dailyRate: userCV.dailyRate || '',
          currency: userCV.currency || 'USD',
        });
        setProfilePhoto(userCV.profilePhoto || null);
        setExperiences(userCV.experiences || []);
        setEducation(userCV.education || []);
        setLanguages(userCV.languages || []);
        setCertifications(userCV.certifications || []);
      } else {
        // No CV exists, go to edit/create mode
        setCvExists(false);
        setIsEditing(true);
      }
    };
  ...
  const handleCVChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        toast.error(t('experts.createAccount.validation.cvInvalidType'));
        return;
      }
      if (file.size > 10 * 1024 * 1024) {
        toast.error(t('experts.createAccount.validation.cvTooLarge'));
        return;
      }
      setCvFile(file);
      toast.success(t('experts.createAccount.validation.cvSelected'));
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    // 9. CV file obligatoire (seulement si le CV n'existe pas déjà)
    if (!cvExists && !cvFile) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Save CV to database service
    if (cvExists) {
      // Update existing CV
      expertsDataService.updateCurrentUserCV({
        profilePhoto,
        cvFile,
      ...
      toast.success(t('monEspace.myCV.updateSuccess'));
    } else {
      // Create new CV
      expertsDataService.createCurrentUserCV({
        profilePhoto,
        cvFile,
        firstName: formData.firstName,
        lastName: formData.lastName,
      ...
      setCvExists(true);
      toast.success(t('monEspace.myCV.createSuccess'));
    }
  ...
  const handleCancel = () => {
    if (!cvExists) {
      // If CV doesn't exist, keep in edit mode but reset form
      const confirmReset = window.confirm(t('monEspace.myCV.confirmCancel'));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <PageBanner
        title={cvExists && !isEditing ? t('monEspace.banner.myCV.title') : t('monEspace.banner.myCV.createTitle')}
        description={cvExists && !isEditing ? t('monEspace.banner.myCV.description') : t('monEspace.banner.myCV.createDescription')}
        icon={FileUser}
      />
      ...
      <PageContainer className="my-6">
        <div className="px-4 sm:px-5 lg:px-6 py-6">
          {/* Edit Button for existing CV */}
          {cvExists && !isEditing && (
            <div className="flex justify-end mb-6">
              <Button onClick={() => setIsEditing(true)}>
                <FileText className="w-4 h-4 mr-2" />
                {t('monEspace.myCV.edit')}
              </Button>
            </div>
          )}
                  ...
                  <div key={exp.id} className="border rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2 md:col-span-2">
                        ...
                        />
                        {validationErrors[`experience_${exp.id}_title`] && (
                          <p className="text-sm text-red-500">{validationErrors[`experience_${exp.id}_title`]}</p>
                        )}
                      </div>
                      <div className="space-y-2">
                        ...
                        {validationErrors[`experience_${exp.id}_company`] && (
                          <p className="text-sm text-red-500">{validationErrors[`experience_${exp.id}_company`]}</p>
                        )}
                      </div>
                      <div className="space-y-2">
                        ...
                        {validationErrors[`experience_${exp.id}_location`] && (
                          <p className="text-sm text-red-500">{validationErrors[`experience_${exp.id}_location`]}</p>
                        )}
                      </div>
                      <div className="space-y-2">
                        ...
                        {validationErrors[`experience_${exp.id}_startDate`] && (
                          <p className="text-sm text-red-500">{validationErrors[`experience_${exp.id}_startDate`]}</p>
                        )}
                      </div>
                      <div className="space-y-2">
                        ...
                        {validationErrors[`experience_${exp.id}_endDate`] && (
                          <p className="text-sm text-red-500">{validationErrors[`experience_${exp.id}_endDate`]}</p>
                        )}
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2 md:col-span-2">
                        ...
                        {validationErrors[`education_${edu.id}_degree`] && (
                          <p className="text-sm text-red-500">{validationErrors[`education_${edu.id}_degree`]}</p>
                        )}
                      </div>
                      <div className="space-y-2 md:col-span-2">
                        ...
                        {validationErrors[`education_${edu.id}_institution`] && (
                          <p className="text-sm text-red-500">{validationErrors[`education_${edu.id}_institution`]}</p>
                        )}
                      </div>
                      <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        ...
                        {validationErrors[`education_${edu.id}_year`] && (
                          <p className="text-sm text-red-500">{validationErrors[`education_${edu.id}_year`]}</p>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div key={lang.id} className="flex gap-4 items-end">
                    <div className="flex-1 space-y-2">
                      ...
                      {validationErrors[`language_${lang.id}_name`] && (
                        <p className="text-sm text-red-500">{validationErrors[`language_${lang.id}_name`]}</p>
                      )}
                    </div>
                    <div className="flex-1 space-y-2">
                      ...
                      >
                        <SelectTrigger className={validationErrors[`language_${lang.id}_level`] ? 'border-red-500' : ''}>
                          <SelectValue placeholder={t('experts.editProfile.selectLevel')} />
                        </SelectTrigger>
                        <SelectContent>
                      ...
                      {validationErrors[`language_${lang.id}_level`] && (
                        <p className="text-sm text-red-500">{validationErrors[`language_${lang.id}_level`]}</p>
                      )}
                    </div>
                    {isEditing && (
                      <Button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2 md:col-span-2">
                        ...
                        {validationErrors[`certification_${cert.id}_name`] && (
                          <p className="text-sm text-red-500">{validationErrors[`certification_${cert.id}_name`]}</p>
                        )}
                      </div>
                      <div className="space-y-2 md:col-span-2">
                        ...
                        {validationErrors[`certification_${cert.id}_issuer`] && (
                          <p className="text-sm text-red-500">{validationErrors[`certification_${cert.id}_issuer`]}</p>
                        )}
                      </div>
                      <div className="space-y-2">
                        ...
                        {validationErrors[`certification_${cert.id}_date`] && (
                          <p className="text-sm text-red-500">{validationErrors[`certification_${cert.id}_date`]}</p>
                        )}
                      </div>
                      <div className="space-y-2">
            ...
            {isEditing && (
              <div className="flex justify-end gap-4">
                ...
                <Button type="submit">
                  ...
                  {cvExists ? t('monEspace.myCV.saveChanges') : t('monEspace.myCV.createCV')}
                </Button>
              </div>
            )}
          </form>
