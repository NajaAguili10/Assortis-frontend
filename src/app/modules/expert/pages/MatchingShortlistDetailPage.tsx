...
export default function MatchingShortlistDetailPage() {
  const { id } = useParams<{ id: string }>();
  const { t } = useTranslation();
  const { getOpportunityById } = useMatchingOpportunities();

  const opportunity = id ? getOpportunityById(id) : undefined;
  if (!opportunity || opportunity.type !== OpportunityTypeEnum.SHORTLIST) {
    return <Navigate to="/matching-opportunities/opportunities" replace />;
  }

  return (
    <div className="min-h-screen bg-gray-50">
      <PageBanner icon={Target} title={opportunity.title} description={t('matching-opportunities.types.shortlist')} />
      <PageContainer>
        <MatchingOpportunitiesSubMenu />
        <div className="mt-8">
          <MatchingOpportunityDetailContent opportunity={opportunity} variant="shortlist" />
        </div>
      </PageContainer>
    </div>
  );
