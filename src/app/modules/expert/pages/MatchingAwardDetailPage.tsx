...
export default function MatchingAwardDetailPage() {
  const { id } = useParams<{ id: string }>();
  const { t } = useTranslation();
  const { getOpportunityById } = useMatchingOpportunities();

  const opportunity = id ? getOpportunityById(id) : undefined;
  if (!opportunity || opportunity.type !== OpportunityTypeEnum.CONTRACT_AWARD) {
    return <Navigate to="/matching-opportunities/opportunities" replace />;
  }

  return (
    <div className="min-h-screen bg-gray-50">
      <PageBanner icon={Target} title={opportunity.title} description={t('matching-opportunities.types.contractAward')} />
      <PageContainer>
        <MatchingOpportunitiesSubMenu />
        <div className="mt-8">
          <MatchingOpportunityDetailContent opportunity={opportunity} variant="award" />
        </div>
      </PageContainer>
    </div>
  );
