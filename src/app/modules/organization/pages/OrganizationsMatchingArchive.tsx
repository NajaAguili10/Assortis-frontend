...
export default function OrganizationsMatchingArchive() {
  ...
  // Merge translations
  const translations = {
    ...organizationsMatchingTranslations[language],
  };
  ...
  const loadSavedMatchings = () => {
    try {
      const saved = localStorage.getItem('organizationsMatchingArchive');
      if (saved) {
  ...
  const stats = useMemo(() => {
    ...
    const totalOrganizations = savedMatchings.reduce((sum, m) => sum + m.totalOrganizations, 0);
    const avgScore = totalOrganizations > 0
      ? Math.round(savedMatchings.reduce((sum, m) => sum + (m.avgScore * m.totalOrganizations), 0) / totalOrganizations)
      : 0;
  ...
  const handleDeleteMatching = () => {
    ...
    try {
      localStorage.setItem('organizationsMatchingArchive', JSON.stringify(updatedMatchings));
    } catch (error) {
      console.error('Error saving to localStorage:', error);
    }
    
    toast.success(translations['organizations.matchingDossier.card.deleteSuccess']);
  ...
  // Get contact history for an organization
  const getOrganizationContactHistory = (orgId: string) => {
    try {
      const history = JSON.parse(localStorage.getItem('projectContactHistory') || '[]');
      return history
        .filter((h: any) => h.projectId === orgId)
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <PageBanner
        title={translations['organizations.matchingDossier.title']}
        description={translations['organizations.matchingDossier.subtitle']}
        icon={Archive}
        stats={[
          { value: stats.total.toString(), label: translations['organizations.matchingDossier.stats.totalSaved'] },
        ]}
      />
      ...
      <PageContainer className="my-6">
        <div className="px-4 sm:px-5 lg:px-6 py-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-6">
            <StatCard
              title={translations['organizations.matchingDossier.stats.totalSaved']}
            ...
            />
            <StatCard
              title={translations['organizations.matchingDossier.stats.thisMonth']}
            ...
            />
            <StatCard
              title={translations['organizations.matchingDossier.stats.highMatches']}
            ...
            />
            <StatCard
              title={translations['organizations.matchingDossier.stats.avgScore']}
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="relative">
                ...
                <Input
                  type="text"
                  placeholder={translations['organizations.matchingDossier.list.searchPlaceholder']}
                  value={searchQuery}
              ...
              <div>
                <Select value={dateFilter} onValueChange={setDateFilter}>
                  ...
                  <SelectContent>
                    <SelectItem value="ALL">{translations['organizations.matchingDossier.list.allDates']}</SelectItem>
                    <SelectItem value="WEEK">{translations['organizations.matchingDossier.list.thisWeek']}</SelectItem>
                    <SelectItem value="MONTH">{translations['organizations.matchingDossier.list.thisMonth']}</SelectItem>
                    <SelectItem value="YEAR">{translations['organizations.matchingDossier.list.thisYear']}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
          ...
          {filteredMatchings.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-lg border">
              ...
              <h3 className="text-lg font-semibold text-primary mb-2">
                {translations['organizations.matchingDossier.list.empty']}
              </h3>
              <p className="text-muted-foreground mb-4">
                {translations['organizations.matchingDossier.list.emptyMessage']}
              </p>
                ...
                <Card key={matching.id} className="p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        <span>
                          {translations['organizations.matchingDossier.card.savedOn']}{' '}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4 p-4 bg-gray-50 rounded-lg">
                    <div className="text-center">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {translations['organizations.matchingDossier.card.organizations']}
                      </div>
                    </div>
                    <div className="text-center">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {translations['organizations.matchingDossier.card.avgMatch']}
                      </div>
                    </div>
                    <div className="text-center">
                  ...
                  {matching.results.length > 0 && (
                    <div className="mb-4 p-3 bg-blue-50 rounded-lg">
                      <h4 className="text-sm font-semibold text-primary mb-2">Sample Organizations:</h4>
                      <div className="space-y-1">
                        {matching.results.slice(0, 2).map((org) => (
                          <div key={org.id} className="flex items-center justify-between text-sm">
                            <span className="text-muted-foreground truncate flex-1">{org.name}</span>
                            <Badge variant="secondary" className="ml-2">
                              {org.matchScore}%
                            </Badge>
                          </div>
                        ))}
                        {matching.totalOrganizations > 2 && (
                          <p className="text-xs text-muted-foreground mt-1">
                            + {matching.totalOrganizations - 2} more organizations
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {expandedMatchingId === matching.id ? (
                          ...
                          <>
                            ...
                            {translations['organizations.matchingDossier.card.viewResults']}
                          </>
                        )}
                      </Button>
                      <Button
                      ...
                      >
                        ...
                        {translations['organizations.matchingDossier.card.delete']}
                      </Button>
                    </div>
                  </div>
                  ...
                  {expandedMatchingId === matching.id && (
                    <div className="mt-6 pt-6 border-t space-y-4">
                      ...
                      <div className="space-y-3" key={refreshKey}>
                        <h4 className="text-sm font-semibold text-gray-900">All Matching Organizations:</h4>
                        {matching.results.map((org: any) => {
                          const contactHistory = getOrganizationContactHistory(org.id);
                          
                          return (
                          <Card key={org.id} className="p-4 hover:shadow-md transition-shadow">
                            <div className="flex items-start justify-between gap-4">
                              <div className="flex-1">
                                <h5 className="font-semibold text-gray-900 mb-1">{org.name}</h5>
                                <div className="text-sm text-gray-600 mb-2">{org.description}</div>
                                ...
                                <div className="flex flex-wrap items-center gap-3 text-sm">
                                  ...
                                  <div className="flex items-center gap-1 text-gray-500">
                                    ...
                                    <span>{org.budgetRange?.min?.toLocaleString()} - {org.budgetRange?.max?.toLocaleString()} {org.currency}</span>
                                  </div>
                                  <div className="flex items-center gap-1 text-gray-500">
                                  ...
                                  <div className="flex items-center gap-1 text-gray-500">
                                    ...
                                    <span>{org.experience} years</span>
                                  </div>
                                </div>
                                  ...
                                  <div className="mt-4 pt-4 border-t">
                                    <div className="flex items-center gap-2 mb-3">
                                      ...
                                      <h6 className="text-sm font-semibold text-gray-900">
                                        {translations['organizations.contact.history.title']}
                                      </h6>
                                        ...
                                        >
                                          <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-xs text-gray-500">
                                              {translations['organizations.contact.history.sentOn']}{' '}
                                          ...
                                          <div className="space-y-1">
                                            <p className="text-sm">
                                              <span className="font-medium text-gray-700">
                                                {translations['organizations.contact.history.subject']}
                                              </span>{' '}
                                              <span className="text-gray-900">{contact.subject}</span>
                                            </p>
                                            <p className="text-sm">
                                              <span className="font-medium text-gray-700">
                                                {translations['organizations.contact.history.message']}
                                              </span>{' '}
                              ...
                              <div className="flex flex-col items-center gap-2">
                                ...
                                >
                                  ...
                                  {translations['organizations.contact.buttonContact']}
                                </Button>
                              </div>
                            </div>
                          </Card>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{translations['organizations.matchingDossier.card.deleteConfirm']}</AlertDialogTitle>
            <AlertDialogDescription>
