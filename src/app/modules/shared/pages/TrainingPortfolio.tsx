...
export default function TrainingPortfolio() {
  ...
  useEffect(() => {
    const loadPortfolioData = async () => {
      ...
      try {
        ...
        const safeCompletedTrainings = completedTrainingsData || [];
        const safeCertifications = certificationsData || [];

        setStats({
          completedTrainings:
              statsData.completedTrainings ?? safeCompletedTrainings.length,
          certifications:
              statsData.certifications ?? safeCertifications.length,
          achievements: statsData.achievements ?? 0,
        });

        setCompletedTrainings(safeCompletedTrainings);
        setCertifications(safeCertifications);
      } catch (err: any) {
        setError(
            err.message || 'Unable to load training portfolio data'
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleViewTrainingCertificate = (
    ...
    const fileUrl = getCertificateFileUrl(certification);
  ...
  const handleDownloadTrainingCertificate = async (
    ...
    try {
      await downloadCertification(
          fileUrl,
          getTrainingCertificateFileName(training, certification)
      );
    } catch (err: any) {
      setError(
          err.message || 'Unable to download certificate'
      );
    } finally {
      ...
      <div className="min-h-screen">
        ...
        <PageContainer className="my-6">
          <div className="px-4 sm:px-5 lg:px-6 py-6 space-y-6">
                              ...
                              {training.certificationAvailable && (
                                  <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <Button
                                        ...
                                        variant="outline"
                                        onClick={() =>
                                            handleViewTrainingCertificate(training)
                                        }
                                        disabled={!fileUrl}
                                    >
                                    ...
                                    <Button
                                        ...
                                        onClick={() =>
                                            handleDownloadTrainingCertificate(training)
                                        }
            ...
            <div className="bg-white rounded-lg border p-5">
              <h2 className="text-lg font-semibold text-primary mb-4">
                {t('training.certificatesHistory.title')}
              </h2>
              ...
              {loading ? (
                  ...
                  <p className="text-muted-foreground">
                    {t('training.certifications.noResults.message')}
                  </p>
              ) : (
                          ...
                          >
                            ...
                            <div className="space-y-2 text-sm text-muted-foreground mb-4">
                              ...
                              {certification.completedOn && (
                                  <p className="flex items-center gap-2">
                                    ...
                                    : {formatDate(certification.completedOn)}
                                  </p>
                              )}
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              <Button
                                  ...
                                  variant="outline"
                                  onClick={() =>
                                      handleViewCertificate(certification)
                                  }
                                  disabled={!fileUrl}
                              >
                                <Award className="w-4 h-4 mr-2" />
                              ...
                              <Button
                                  ...
                                  className="bg-[#B82547] hover:bg-[#a01f3c] text-white"
                                  onClick={() =>
                                      handleDownloadCertificate(certification)
                                  }
                                  disabled={!fileUrl || isDownloading}
                              >
                                <Download className="w-4 h-4 mr-2" />
