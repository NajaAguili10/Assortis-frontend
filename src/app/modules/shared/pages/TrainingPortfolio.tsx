...
export default function TrainingPortfolio() {
  const { t } = useLanguage();
  const { completedTrainings, certificatesEarned } = useTrainingCommerce();
        ...
        <div className="px-4 sm:px-5 lg:px-6 py-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-lg border p-4">
              ...
              <p className="text-2xl font-bold text-primary">{certificatesEarned.length}</p>
            </div>
            <div className="bg-white rounded-lg border p-4">
              <p className="text-sm text-muted-foreground">{t('training.portfolio.achievements')}</p>
              <p className="text-2xl font-bold text-primary">{Math.max(certificatesEarned.length, 1)}</p>
            </div>
          </div>
